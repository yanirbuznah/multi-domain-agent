...
class Executive(Executor):
    ...
    def initialize(self, services):
        ...
        for state in list(self.finish_states.keys()):
            if self.get_number_of_uncompleted_goals(self.services.goal_tracking.uncompleted_goals[0],
                                                    self.finish_states[state]) > 0:
    ...
    def get_probabilistic_per_state(self, option, probs, states_and_probs):
        ...
        if probs[0] == 1:
            ...
            states_and_probs['r'] = states_and_probs['q'] = self.get_reward(
                self.services.goal_tracking.uncompleted_goals[0], state)
        ...
        for i in range(10):
            ...
            if states_from_probs[index] == False:
                ...
                reward_from_actions[index] = probs[index] * self.get_reward(
                    self.services.goal_tracking.uncompleted_goals[0], state)
                hash_state = make_hash_sha256(state)
    ...
    def mc_control(self, hash_state, k, t):
        ...
        for i in range(t):
            ...
            state_rewards += self.mc_control(self.get_state_from_action(self.model[hash_state]['actions'][option]),
                                             k - 1, t)
    ...
    def pick_best_option(self, options, hash_state, reward):
        ...
        if len(self.model[hash_state]['actions'][option]) < len(probs) + 2:
            probs_per_states = self.get_probabilistic_per_state(option, probs,
                                                                self.model[hash_state]['actions'][option])
            self.model[hash_state]['actions'][option] = probs_per_states
        else:
