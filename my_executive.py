    ...
    def run(self):
        if self.flag == '-E':
            if self.data['count'] == 0 or self.data['planning_success']:
                executor = LocalSimulator().run(domain,problem,LearningFromPlanExecutor(self.deterministic_domain,"policy_maze"))
                if executor.success:
                    print executor
                    return True
            ...
            if self.data['count'] == 0:
                LocalSimulator().run(domain, problem, LearningFromPlanExecutor(self.deterministic_domain, "policy_maze"))
            for i in range(10):
